...
def secret_view(request, secret_id):
    if request.user.is_authenticated:
        ...
        if secret_id in list_of_secid:
            ...
            # second secret by mail
            send_mail(
                'Here is your old secret of'+cur_secret.get_site(),
                'Nice to meet you! Here is Pwdmanager, you have been requested to view the secret of ' + cur_secret.get_site() + ', make sure in this email: username: ' + cur_secret.get_username() + ', email: ' + cur_secret.get_email() + ', password: ' + cur_secret.get_password() + ', notes: ' + cur_secret.get_notes() + '. Good luck!',
                'pwdmanager@example.com',
                [ request.user.email ],
                fail_silently=False,
            )

            secret_message = "Mail of your secret has been sent!"
        ...
        return HttpResponse("You are still not authenticated!")
